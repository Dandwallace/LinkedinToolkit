...
export const COLUMNS = [
  { key: 'date', label: 'Date', re: /^(date|day|start\s*date|date\s*\(.*\))$/i },
  { key: 'campaign', label: 'Campaign', re: /^campaign\s*(name)?$/i },
  { key: 'campaignId', label: 'Campaign ID', re: /campaign\s*id/i },
  { key: 'campaignGroup', label: 'Campaign group', re: /campaign\s*group/i },
...
export function normaliseStatus(v) {
  ...
  if (/ARCHIV|CANCEL/.test(s)) return 'ARCHIVED';
  return s;
}
...
export function parseExport(text, { encoding = null } = {}) {
  ...
  if (headerIdx < 0) {
    return {
      ok: false,
      error:
        'No recognisable header row. Export from Campaign Manager without editing it first — ' +
  ...
  for (const r of grid.slice(headerIdx + 1)) {
     ...
     * usually the date, not the campaign — so both are checked. */
    if (isTotalsRow(r, campaign)) continue;

    const d = toDate(cell(r, 'date'));
    if (!d) undated++;
    ...
    rows.push({
      ...
      campaignGroup: String(cell(r, 'campaignGroup') || '').trim() || null,
