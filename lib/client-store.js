 ...
 * uploaded.
 *
 * A client holds one report per report type, not one report. Export types
 * carry different columns and describe different things: a Performance
 * report and a Delivery report cannot be combined into a single dataset,
 * and merging their rows produces figures that are true of neither. So each
 * type is stored under its own key, the most recent of each is kept, and
 * the dashboard reads each figure from whichever report actually carries
 * it.
 */

import { storage } from '@/lib/storage';
import { reportTypeKey } from '@/lib/parse-export';
...
/** Spend, the way it is written everywhere it is read at a glance. */
export const money = (n) => `$${Math.round(Number(n) || 0).toLocaleString('en-GB')}`;
...
/** What an export whose preamble was stripped gets filed under. */
export const UNKNOWN_REPORT_TYPE = 'Unspecified report';

/** Reads the raw stored blob for a client, in either shape. */
async function readRecord(clientId) {
  try {
    const r = await storage.get(KEY(clientId));
    const parsed = JSON.parse(r.value);
    if (parsed?.reports) return parsed;
    /* Records written before reports were split by type are a single blob.
     * Wrap the old shape rather than discarding it, so an upload from
     * before this change still shows on the dashboard. */
    if (parsed?.campaigns) {
      const type = parsed.reportType || UNKNOWN_REPORT_TYPE;
      return { clientId, reports: { [reportTypeKey(type)]: { ...parsed, type } } };
    }
    return null;
  } catch {
    return null;
...
/**
 * Files a parsed export against a client under its own report type.
 *
 * Uploading a type that is already held replaces that type and leaves the
 * others alone, so a fresh Performance export never wipes the Delivery
 * report sitting beside it. Within one type it is still a replace rather
 * than a merge: two exports of overlapping date ranges would double-count,
 * and there is no reliable key to de-duplicate daily rows on.
 */
export async function saveClientData(clientId, parsed, meta = {}) {
  ...
  const report = {
    ...
    /* Which questions this report can answer, recorded at save time so the
     * dashboard never has to guess from the shape of the rows. */
    hasStatus: parsed.hasStatus ?? campaigns.some((c) => Boolean(c.status)),
    hasSpend: parsed.hasSpend ?? Number(parsed.totals?.spend) > 0,
    campaigns,
    groups,
  };

  const existing = (await readRecord(clientId)) || { clientId, reports: {} };
  const record = {
    clientId,
    reports: { ...existing.reports, [reportTypeKey(type)]: report },
  };
  await storage.set(KEY(clientId), JSON.stringify(record));
  return record;
}

/** Health is recomputed on read, so a threshold change needs no re-upload. */
const rehealth = (list) =>
  (list || []).map((c) => {
    const h = healthOf(c);
...
/**
 * Returns one client's stored reports plus the figures derived from them.
 *
 * Nothing is merged across types. Status counts come from whichever report
 * carries a status column, spend from whichever carries spend, and the two
 * may be different reports of different dates. `reports` lists everything
 * held so the UI can show when one is stale relative to another.
 */
export async function loadClientData(clientId) {
  const record = await readRecord(clientId);
  const held = Object.values(record?.reports || {});
  if (!held.length) return null;

  const reports = held
    .map((r) => {
      const campaigns = rehealth(r.campaigns);
      return {
        ...r,
        type: r.type || UNKNOWN_REPORT_TYPE,
        labels: r.labels || { group: 'Campaign group', unit: 'Campaign' },
        campaigns,
        /* Records saved before the two-level model have no groups array. */
        groups: r.groups ? rehealth(r.groups) : [],
        /* Older records predate these being recorded at save time. */
        hasStatus: r.hasStatus ?? campaigns.some((c) => Boolean(c.status)),
        hasSpend: r.hasSpend ?? Number(r.totals?.spend) > 0,
      };
    })
    .sort(byNewest);

  /* Counts come from a report that carries status. When none does, the
   * newest still describes the campaigns; it just cannot say which are
   * paused, so everything in it reads as running. Showing no counts at all
   * in that case looks like a failure rather than a missing column. */
  const withStatus = reports.find((r) => r.hasStatus) || null;
  const statusReport = withStatus || reports[0];
  const spendReport =
    reports.find((r) => r.hasSpend && Number(r.totals?.spend) > 0) ||
    reports.find((r) => r.hasSpend) ||
    reports[0];
  /* The report the card's labels and date range come from. Status first,
   * because that is the one the counts are drawn from. */
  const primary = statusReport || spendReport || reports[0];

  const campaigns = statusReport ? statusReport.campaigns : [];
  const groups = statusReport ? statusReport.groups : [];

  return {
    clientId,
    reports,
    statusReport,
    /* True when no report carries a status column, so the counts above are
     * everything the newest report knows about rather than live states. */
    statusUnknown: !withStatus,
    spendReport,
    ...
    /* Total spend for the client, from the one report that carries it. */
    spend: Number(spendReport?.totals?.spend) || 0,
  };
}

/** Clears one client's stored reports. The other clients are untouched. */
export async function clearClientData(clientId) {
  await storage.delete(KEY(clientId));
}

/** Clears every client's stored reports. */
export async function clearAllClientData() {
  for (const c of CLIENTS) await storage.delete(KEY(c.id));
}
...
export async function loadAllClients() {
  ...
  for (const c of CLIENTS) {
    ...
    out.push({
      ...c,
      data,
      counts: data ? data.counts : countsOf([]),
      spend: data ? data.spend : 0,
    });
  }
  return out;
}

/** Whole-account totals for the dashboard hero. */
export function combineClients(clients = []) {
  const counts = Object.fromEntries(STATES.map((s) => [s, 0]));
  let total = 0;
  let spend = 0;
  for (const c of clients) {
    for (const s of STATES) counts[s] += c.counts?.[s] || 0;
    total += c.counts?.total || 0;
    spend += c.spend || 0;
  }
  return { ...counts, total, spend };
}
